...
public class Ball extends Performance{
    ...
    public void restore(BallMemento bm){
        strategy = bm.getStrategy();
        xPosition = bm.getPosX();
        yPosition = bm.getPosY();
        xVelocity = bm.getVelX();
        yVelocity = bm.getVelY();
        isActive = bm.getIsActive();
    }

    @Override
    public BallMemento save() {
        return new BallMemento(this.xPosition, this.yPosition, this.xVelocity, this.yVelocity, this.isActive, this.strategy.copy());
    }
